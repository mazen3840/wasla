...
class ClickByCountry extends StatefulWidget {
  ...
  const ClickByCountry({super.key, required this.controller});
...
class _ClickByCountryState extends State<ClickByCountry> {
  ...
  void initState() {
    ...
    dataMap = convertIntMapToDouble(
        widget.controller.ReportData!.data.statistics.clicks);
    colorList = generateRandomColorList(
        widget.controller.ReportData!.data.statistics.clicksCount!);
  }
  ...
  Widget build(BuildContext context) {
    ...
    dataMap = convertIntMapToDouble(bannModel.clicks);
    return Container(
      ...
      child: Column(
        children: [
          itemList(
            width,
            'Click by Country',
            bannModel.clicksCount.toString(),
          ),
          ...
          PieChart(
            ...
            totalValue: widget
                .controller.ReportData!.data.statistics.clicksCount!
                .toDouble(),
          ),
        ],
      ),
